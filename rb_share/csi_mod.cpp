...
bool ref_design_csi_mod::run_once()
{
    ...
    // size_t tti
    // double avg_rb
    // double total_rb_allocated
    //std::vector<double> total_slice_tp;
    //rb_share(total_csi_vec, total_slice_tp, tti, avg_rb, total_rb_allocated);
    _calculate_weights();
...
{
    //size_t est_idx_sched = 0;
    //sklk_phy_csi_vec total_csi_vec;
    for (size_t resource_blk_no = 0; resource_blk_no < _num_resouce_blks; resource_blk_no++) {
        /*for (const auto &[_, ue_radio] : ue_radio_map) {
            ...
            if (ue_radio_container and ue_radio_container->csi[resource_blk_no].ready()) {
                total_csi_vec.insert(total_csi_vec.begin(), ue_radio_container->csi[resource_blk_no][est_idx_sched].data());
	    }
	}*/
...
void ref_design_csi_mod::_calculate_weights(size_t resource_blk_no, bool is_downlink)
{
    ...
    for (const auto &[_, ue_radio] : ue_radio_map) {
        ...
        if (ue_radio_container and ue_radio_container->csi[resource_blk_no].ready()) {
            ...
            //const sklk_phy_csi_vec &user_csi_vec = ue_radio_container->csi[resource_blk_no][est_idx].data();
	    //total_csi_vec = sklk_phy_csi_vec;
	}
    }
